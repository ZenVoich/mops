...
import {formatDir, formatGithubDir, getHighestVersion, readConfig} from '../mops.js';
...
export async function sources({verbose} = {}) {
	...
	let collectDeps = async (config, isRoot = false) => {
		for (const pkgDetails of Object.values(config.dependencies || {})) {
			...
			if (isRoot || !packages[name] || compareVersions(packages[name].version, version) === -1) {
			...
			if (repo){
				const dir = formatGithubDir(name, version) + '/mops.toml';
	...
	for (let [name, {repo, version}] of Object.entries(packages)) {
		let pkgDir;
		if (repo){
			pkgDir =  path.relative(process.cwd(), path.join(root, `.mops/_github/${name}@${version}/src`));
		}else{
